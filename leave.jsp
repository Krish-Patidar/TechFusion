...
<div class="container header">
    <center><h2>Manage Leave</h2></center>
    <br><br>
    <div class="row">
        <%
            ...
            try {
                ...
                while (resultSet.next()) {
                       ...
                       int leaveStatus = resultSet.getInt("leave_status"); // Fetch leave status from DB
        ...
        %>
                     ...
                     <button class="btn btn-success" onclick="updateLeaveStatus(<%= id %>, 1)">Grant Leave</button>
                    <button class="btn btn-danger" onclick="updateLeaveStatus(<%= id %>, 0)">Cancel Leave</button>                   
                                 <span id="leave-logo-<%= id %>" class="leave-logo">L</span>
                            </div>
                        </div>
                    </div>
        <%
  ...
  function updateLeaveStatus(employeeId, status) {
            $.ajax({
                ...
                type: 'POST',
                data: { id: employeeId, leaveStatus: status },
                success: function(response) {
                    const logo = document.getElementById("leave-logo-" + employeeId);
                    if (status === 1) {
                        logo.style.display = "block";
                    } else {
                        logo.style.display = "none";
